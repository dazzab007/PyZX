...
# Initialization of the table of addresses of pixel and attribute lines
...
def init():
    ...
    zx_screen_with_border = pygame.surface.Surface((FULL_SCREEN_WIDTH, FULL_SCREEN_HEIGHT), pygame.HWSURFACE)
    #zx_screen_with_border.set_palette(COLORS)

    screen = pygame.display.set_mode((FULL_SCREEN_WIDTH * ratio, FULL_SCREEN_HEIGHT * ratio), pygame.HWSURFACE | pygame.DOUBLEBUF)
...
def update():
    ...
    # If border has changed, then change it.
    if Z80.ports.current_border != old_border:
        zx_screen_with_border.fill(COLORS[Z80.ports.current_border])
